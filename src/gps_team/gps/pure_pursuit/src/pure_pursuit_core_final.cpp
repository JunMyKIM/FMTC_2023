...
bool nine_cw = false;
bool ten_cw = false;
bool eleven_cw = false;
bool twelve_cw = false;
...
int deliverySign = 0;
    ...
    //좌회전 신호 :: 1,4,5 
///1011 FMTC
const float tf_coord1[2] = {0,0}; //직진신호 정지 시점
////1011 FMTC
const float tf_coord2[2] = {931290.6788326793, 1929778.427274119}; //////////////////////////////FMTC


const float tf_coord3[2] = {0,0};
const float tf_coord4[2] = {0,0};
const float tf_coord5[2] = {0,0};
const float tf_coord6[2] = {0,0};
const float tf_coord7[2] = {0,0};
...
////1011 FMTC
const float slow_down_tf_coord1[2] = {0,0}; 
////1011 FMTC
const float slow_down_tf_coord2[2] = {931296.7066923674, 1929773.7621992668};/////////////////////////////////////////////////FMTC


// // Positions where car should slow down before traffic lights

const float slow_down_tf_coord3[2] = {0,0};
const float slow_down_tf_coord4[2] = {0,0};
const float slow_down_tf_coord5[2] = {0,0};
const float slow_down_tf_coord6[2] = {0,0};
const float slow_down_tf_coord7[2] = {0,0};
...
// mode 6
const float cw_coord_1[2] = {931404.1090683833, 1929885.949613964};//A1
const float cw_coord_2[2] = {931400.9803016452, 1929882.3805002435};//A2

// mode 14
const float cw_coord_3[2] = {931257.4186899525, 1929803.4537292006};//비보호 우회전_1
const float cw_coord_4[2] = {931251.0203120135, 1929811.5169602702};//비보호 우회전_2

//mode 101
// const float cw_coord_5[2] = {931337.5240105817, 1929923.7533899022};//B1

const float cw_coord_5[2] = {931335.5863217736, 1929852.80549464};// 트렉시작전


const float cw_coord_6[2] = {931343.0174317461, 1929921.9552534018};//B2

const float cw_coord_7[2] = {931267.1811840339, 1929843.7260818032};//
const float cw_coord_8[2] = {931275.9922945789, 1929854.4601991293};//
const float cw_coord_9[2] = {931284.7349132496, 1929865.4833592454};//
const float cw_coord_10[2] = {931299.946087555, 1929885.606764571};//
const float cw_coord_11[2] = {931309.6588445924, 1929898.308421503};//
const float cw_coord_12[2] = {931317.124986503, 1929908.1105552067};//




//const float cw_coord_6[2] = {931341.9926191644, 1929922.2521414883};
...
void PurePursuitNode::run(char** argv) {
  ...
  while (ros::ok()) {
    ...
    if (!index_flag) {
      ...
      slow_down_tf_idx_2 = pp_.getPosIndex(slow_down_tf_coord2[0] , slow_down_tf_coord2[1]);//직진 신호/////////////////////////FMTC
      ...
      cw_idx_5 = pp_.getPosIndex(cw_coord_5[0], cw_coord_5[1]);
      cw_idx_6 = pp_.getPosIndex(cw_coord_6[0], cw_coord_6[1]);

      cw_idx_7 = pp_.getPosIndex(cw_coord_7[0], cw_coord_7[1]);
      cw_idx_8 = pp_.getPosIndex(cw_coord_8[0], cw_coord_8[1]);
      cw_idx_9 = pp_.getPosIndex(cw_coord_9[0], cw_coord_9[1]);
      cw_idx_10 = pp_.getPosIndex(cw_coord_10[0], cw_coord_10[1]);
      cw_idx_11 = pp_.getPosIndex(cw_coord_11[0], cw_coord_11[1]);
      cw_idx_12 = pp_.getPosIndex(cw_coord_12[0], cw_coord_12[1]);
...
/////////////////////////////////////////////////
    ...
    // MODE 102 : Lane Keeping Assistant 구간
    if (pp_.mode == 102) {
      pp_.mission_flag = 0;
      const_lookahead_distance_ = dynamic_lookahead_distance;//원래 6이였음
      const_velocity_ = 7;  //원래 18 //20일때 LHD = 6
      final_constant = 1.0;


      // if(pp_.reachMissionIdx(cw_idx_7) && !seven_cw){
      //   for(int i = 0; i < 10; i++){
      ...
      if(pp_.reachMissionIdx(cw_idx_8) && !eight_cw){
        for(int i = 0; i < 10; i++){
          publishPurePursuitDriveMsg(can_get_curvature, kappa, 1);
          usleep(100000);
        }
      ...
      // if(pp_.reachMissionIdx(cw_idx_9) && !nine_cw){
      //   for(int i = 0; i < 10; i++){
      //     publishPurePursuitDriveMsg(can_get_curvature, kappa, 1);
      //     usleep(100000);
      //   }
      ...
      if(pp_.reachMissionIdx(cw_idx_10) && !ten_cw){
        for(int i = 0; i < 10; i++){
          publishPurePursuitDriveMsg(can_get_curvature, kappa, 1);
          usleep(100000);
        }
      ...
      // if(pp_.reachMissionIdx(cw_idx_11) && !eleven_cw){
      //   for(int i = 0; i < 10; i++){
      //     publishPurePursuitDriveMsg(can_get_curvature, kappa, 1);
      //     usleep(100000);
      //   }
      ...
      // if(pp_.reachMissionIdx(cw_idx_12) && !twelve_cw){
      //   for(int i = 0; i < 10; i++){
      //     publishPurePursuitDriveMsg(can_get_curvature, kappa, 1);
      //     usleep(100000);
      //   }
    ...
    if (pp_.mode == 0) {
      pp_.mission_flag = 0;
      const_lookahead_distance_ = dynamic_lookahead_distance;//원래 6이였음
      const_velocity_ = 10;  //원래 18 //20일때 LHD = 6
      final_constant = 1.0;

      if(pp_.reachMissionIdx(cw_idx_5) && !fifth_cw){
        for(int i = 0; i < 35; i++){
          publishPurePursuitDriveMsg(can_get_curvature, kappa, 1);
          usleep(100000);
        }
      ...
      // if(pp_.reachMissionIdx(cw_idx_6) && !sixth_cw){
      //   for(int i = 0; i < 35; i++){
      //     publishPurePursuitDriveMsg(can_get_curvature, kappa, 1);
      //     usleep(100000);
      //   }
    ...
    //좌회전 신호 :: 1,4,5 
      ...
      if ((pp_.reachMissionIdx(tf_idx_2)) && !pp_.straight_go_flag) { 
        while(!pp_.straight_go_flag)
        {
          ...
          std::cout<<"pp_.reachMissionIdx(tf_idx_2)"<<pp_.reachMissionIdx(tf_idx_2)<<std::endl;
  ...
  //직진 신호 :: 2,3,6,7
    ...
    // MODE 101 : 비보호 우회전 및 횡단보도 정지///1939~2134(FMTC_2)///1954~2149(FMTC)
    if (pp_.mode == 101) {
      pp_.mission_flag = 0;
      const_lookahead_distance_ = dynamic_lookahead_distance;//원래 5였음
      const_velocity_ = 5; //9월22일에는 7이었음
      final_constant = 1.2;
    ...
    if (pp_.mode == 14) {
      ...
      if(pp_.reachMissionIdx(cw_idx_4) && !fourth_cw){
        for(int i = 0; i < 35; i++){
          publishPurePursuitDriveMsg(can_get_curvature, kappa, 1);
          usleep(100000);
        }
    ...
    { // 1001
      ...
      if ((pp_.mission_flag == 1 || pp_.mission_flag == 2 || pp_.mission_flag == 3) && pp_.is_delivery_obs_stop_detected == 1) {
        ...
        // a_max_index = 1; // 주석 풀고 바꾸기### ### A1(0), A2(1), A3(2) 영상배달A실패본선중간실패 //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    if (pp_.mode == 9) {
        ...
        else if(parking_space_flag==true)
        {
          ...
          // 좌측 후진 원래 6.5->6.6->6.65->6.7->6.73->6.8
          if (result_time < 6.5&& step == 3) {
               std::cout<<result_time<<std::endl;
            
              std::cout<<"스탭3"<<std::endl;
            pulishControlMsg(-7, -30); //-6.5
            continue;
          } 
          ...
          // }
           // 좌측 전진
          if (result_time < 6.5 && step == 4) {
               std::cout<<result_time<<std::endl;
             std::cout<<"스탭7"<<std::endl;
            pulishControlMsg(7, -30);
            continue;
          } else if (6.5<= result_time && step == 4) {
               std::cout<<result_time<<std::endl;
